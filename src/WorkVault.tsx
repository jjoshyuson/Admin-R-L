...
export function WorkVault({ onBack }: { onBack: () => void }) {
  ...
  function recordDeduction() {
    ...
    const next = appendTransaction(transactions, { id: createRandomId('vault-entry'), employeeId: ledgerAction.employee.id, date: today, type: ledgerAction.type, amount: -amount, shiftId: null, note: ledgerNote, createdAt: new Date().toISOString() })
  ...
  return <div className="workvault-shell">
    ...
    {ledgerAction && <div className="wv-modalback"><section className="wv-modal wv-smallmodal"><header><div><p>{ledgerAction.type === 'advance' ? 'DEDUCTION' : 'COMPLETED PAYMENT'}</p><h2>{ledgerAction.type === 'advance' ? 'Add advance' : 'Record payment'}</h2></div><button onClick={() => setLedgerAction(null)}><X /></button></header><div className="wv-modalbody"><p className="wv-balance-note">Current balance <strong>{money(employeeBalance(transactions, ledgerAction.employee.id))}</strong></p><label className="wv-field"><span>AMOUNT</span><input autoFocus inputMode="decimal" value={ledgerAmount} onChange={(event) => setLedgerAmount(event.target.value.replace(/[^0-9.]/g, ''))} placeholder="0.00" /></label><label className="wv-field"><span>NOTE (OPTIONAL)</span><textarea value={ledgerNote} onChange={(event) => setLedgerNote(event.target.value)} /></label></div><footer><button className="wv-confirm is-coral" disabled={!Number(ledgerAmount)} onClick={recordDeduction}>Record deduction</button></footer></section></div>}
  </div>
}
